    ...
    def get_team_player_league_tags_on_player_or_coach(self, token_dict: dict, reference_dict: dict,
        ...
        for league in self.optimization_tool.value['leagues']:
            ...
            if name in reference_dict[league]:
                ...
                tags.append({"type": TagType.TEAM.value, "value": reference_dict[league][name][TagType.TEAM.value],
                             "confidence": method_confidence})
                ...
                tags.append({"type": TagType.SPORT.value, "value": self.util.league_sport_mapping[league],
                             "confidence": method_confidence})
                # TODO Add nhl player positions
                if is_tag_for_player and league != 'NHL':
                    tags.append({"type": TagType.POSITION.value, "value": reference_dict[league][name][TagType.POSITION.value],
                                 "confidence": method_confidence})
                self.set_optimization_tool(TagType.LEAGUE.value)
                break
        return tags
